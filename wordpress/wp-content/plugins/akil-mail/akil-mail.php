<?php
/*
Plugin Name: akil mail
Version: 1.0.0
Plugin URI: https://#
Author: akil
Author URI: https://a2zcart.site50.net
Description: Send email via SMTP from your WordPress Blog Akil Mail
*/
if ( ! function_exists( 'akilmail_admin_default_setup' ) ) {
	function akilmail_admin_default_setup() {		
		
                add_options_page(__('Akil Mail', 'akil_mail'), __('Akil Mail', 'akil_mail'), 'manage_options', 'swpsmtp_settings', 'swpsmtp_settings');
	}
}
if ( ! function_exists ( 'swpsmtp_admin_init' ) ) {
	function swpsmtp_admin_init() {
			load_plugin_textdomain( 'akil_mail', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
...
if ( ! function_exists ( 'swpsmtp_plugin_action_links' ) ) {
	function swpsmtp_plugin_action_links( $links, $file ) {
		...
		if ( $file == $this_plugin ) {
			$settings_link = '<a href="options-general.php?page=swpsmtp_settings">' . __( 'Settings', 'akil_mail' ) . '</a>';
...
if ( ! function_exists ( 'swpsmtp_register_plugin_links' ) ) {
	function swpsmtp_register_plugin_links( $links, $file ) {
		...
		if ( $file == $base ) {
			$links[] = '<a href="options-general.php?page=swpsmtp_settings">' . __( 'Settings', 'akil_mail' ) . '</a>';
		}
		return $links;
	}
}
...
if ( ! function_exists( 'swpsmtp_settings' ) ) {
	function swpsmtp_settings() {
		...
		<div class="swpsmtp-mail wrap" id="swpsmtp-mail">
			...
			<h2><?php _e( "Akil Mail Settings", 'akil_mail' ); ?></h2>
			...
			<div id="swpsmtp-settings-notice" class="updated fade" style="display:none">
				<p><strong><?php _e( "Notice:", 'akil_mail' ); ?></strong> <?php _e( "The plugin's settings have been changed. In order to save them please don't forget to click the 'Save Changes' button.", 'akil_mail' ); ?></p>
			</div>
			<h3><?php _e( 'General Settings', 'akil_mail' ); ?></h3>
			<form id="swpsmtp_settings_form" method="post" action="">					
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php _e( "From Email Address", 'akil_mail' ); ?></th>
						<td>
							<input type="text" name="swpsmtp_from_email" value="<?php echo esc_attr( $swpsmtp_options['from_email_field'] ); ?>"/><br />
					</td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e( "From Name", 'akil_mail' ); ?></th>
						<td>
					...
					<tr class="ad_opt swpsmtp_smtp_options">
						<th><?php _e( 'SMTP Host', 'akil_mail' ); ?></th>
						<td>
							<input type='text' name='swpsmtp_smtp_host' value='<?php echo esc_attr($swpsmtp_options['smtp_settings']['host']); ?>' /><br />
						</td>
					</tr>
					<tr class="ad_opt swpsmtp_smtp_options">
						<th><?php _e( 'Type of Encription', 'akil_mail' ); ?></th>
						<td>
							<label for="swpsmtp_smtp_type_encryption_1"><input type="radio" id="swpsmtp_smtp_type_encryption_1" name="swpsmtp_smtp_type_encryption" value='none' <?php if( 'none' == $swpsmtp_options['smtp_settings']['type_encryption'] ) echo 'checked="checked"'; ?> /> <?php _e( 'None', 'akil_mail' ); ?></label>
							<label for="swpsmtp_smtp_type_encryption_2"><input type="radio" id="swpsmtp_smtp_type_encryption_2" name="swpsmtp_smtp_type_encryption" value='ssl' <?php if( 'ssl' == $swpsmtp_options['smtp_settings']['type_encryption'] ) echo 'checked="checked"'; ?> /> <?php _e( 'SSL', 'akil_mail' ); ?></label>
							<label for="swpsmtp_smtp_type_encryption_3"><input type="radio" id="swpsmtp_smtp_type_encryption_3" name="swpsmtp_smtp_type_encryption" value='tls' <?php if( 'tls' == $swpsmtp_options['smtp_settings']['type_encryption'] ) echo 'checked="checked"'; ?> /> <?php _e( 'TLS', 'akil_mail' ); ?></label><br />
						</td>
					</tr>
					<tr class="ad_opt swpsmtp_smtp_options">
						<th><?php _e( 'SMTP Port', 'akil_mail' ); ?></th>
						<td>
							<input type='text' name='swpsmtp_smtp_port' value='<?php echo esc_attr($swpsmtp_options['smtp_settings']['port']); ?>' /><br />
						</td>
					</tr>
					<tr class="ad_opt swpsmtp_smtp_options">
						<th><?php _e( 'SMTP Authentication', 'akil_mail' ); ?></th>
						<td>
							<label for="swpsmtp_smtp_autentication"><input type="radio" id="swpsmtp_smtp_autentication" name="swpsmtp_smtp_autentication" value='no' <?php if( 'no' == $swpsmtp_options['smtp_settings']['autentication'] ) echo 'checked="checked"'; ?> /> <?php _e( 'No', 'akil_mail' ); ?></label>
							<label for="swpsmtp_smtp_autentication"><input type="radio" id="swpsmtp_smtp_autentication" name="swpsmtp_smtp_autentication" value='yes' <?php if( 'yes' == $swpsmtp_options['smtp_settings']['autentication'] ) echo 'checked="checked"'; ?> /> <?php _e( 'Yes', 'akil_mail' ); ?></label><br />
						</td>
					</tr>
					<tr class="ad_opt swpsmtp_smtp_options">
						<th><?php _e( 'SMTP username', 'akil_mail' ); ?></th>
						<td>
							<input type='text' name='swpsmtp_smtp_username' value='<?php echo esc_attr($swpsmtp_options['smtp_settings']['username']); ?>' /><br />
						</td>
					</tr>
					<tr class="ad_opt swpsmtp_smtp_options">
						<th><?php _e( 'SMTP Password', 'akil_mail' ); ?></th>
						<td>
				...
				<p class="submit">
					<input type="submit" id="settings-form-submit" class="button-primary" value="<?php _e( 'Save Changes', 'akil_mail' ) ?>" />
			...
			<h3><?php _e( 'Testing And Debugging Settings', 'akil_mail' ); ?></h3>
			<form id="swpsmtp_settings_form" method="post" action="">					
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php _e( "To", 'akil_mail' ); ?>:</th>
						<td>
							<input type="text" name="swpsmtp_to" value=""/><br />
						</td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e( "Subject", 'akil_mail' ); ?>:</th>
						<td>
							<input type="text" name="swpsmtp_subject" value=""/><br />
						</td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php _e( "Message", 'akil_mail' ); ?>:</th>
						<td>
				...
				<p class="submit">
					<input type="submit" id="settings-form-submit" class="button-primary" value="<?php _e( 'Send Test Email', 'akil_mail' ) ?>" />
...
add_action( 'admin_menu', 'akilmail_admin_default_setup' );
